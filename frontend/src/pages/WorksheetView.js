...
const SectionTitle = ({ num, title }) => (
  <h3 className="font-heading font-bold text-[#404080] text-lg border-b-2 border-[#404080] pb-1.5 mb-4 mt-8">{num}. {title}</h3>
);
...
export default function WorksheetView() {
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FA]">
      <TopNav right={
        <Button size="sm" onClick={() => window.print()} data-testid="download-pdf-button" className="bg-[#404080] hover:bg-[#29166F] text-white"><Download className="w-4 h-4 sm:mr-1.5" /><span className="hidden sm:inline">Unduh PDF</span></Button>
      } />
      <main className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
        <button onClick={() => navigate(-1)} className="no-print flex items-center gap-1.5 text-sm text-slate-500 hover:text-[#404080] mb-5 transition-colors" data-testid="view-back-button"><ArrowLeft className="w-4 h-4" /> Kembali</button>
        ...
        <div className="print-container bg-white border border-slate-200 rounded-lg shadow-sm p-8 md:p-12" data-testid="worksheet-view">
          ...
          <table className="w-full border-collapse mb-2">
            <thead><tr className="bg-[#EAEAF2] text-[#404080]"><th className={`${cellPad} text-left w-32`}>Kode MK</th><th className={`${cellPad} text-left`}>Nama MK</th><th className={`${cellPad} text-left w-16`}>SKS</th></tr></thead>
          ...
          <div className="overflow-x-auto">
            <table className="w-full border-collapse mb-2">
              <thead>
                <tr className="bg-[#EAEAF2] text-[#404080]"><th rowSpan={2} className={`${cellPad} text-left align-middle`}>Mata Kuliah</th><th colSpan={weeks} className={`${cellPad} text-center`}>Minggu ke- (Jumlah Jam)</th><th rowSpan={2} className={`${cellPad} text-left align-middle`}>Catatan</th></tr>
                <tr className="bg-[#EAEAF2] text-[#404080]">{Array.from({ length: weeks }).map((_, w) => <th key={w} className={`${cellPad} text-center w-8`}>{w + 1}</th>)}</tr>
              </thead>
            ...
            <div key={bi} className="mb-6 print-break">
              ...
              <div className="overflow-x-auto">
                <table className="w-full border-collapse text-xs">
                  <thead><tr className="bg-[#EAEAF2] text-[#404080]"><th className={`${cellPad} w-12`}>Minggu Ke-</th><th className={`${cellPad} text-left`}>Target Belajar</th><th className={`${cellPad} w-24`}>Jumlah Halaman Modul</th><th className={`${cellPad} text-left`}>Media Belajar</th><th className={`${cellPad} w-24`}>Lama Belajar (Jam)</th></tr></thead>
          ...
          {results.length === 0 ? <p className="text-sm text-slate-400">-</p> : results.map((res, i) => (
            <div key={i} className="mb-5 border border-slate-200 rounded-md overflow-hidden print-break">
              <div className="px-4 py-2 bg-slate-50 border-b border-slate-100 text-sm">
                ...
                {res.minggu && <span className="ml-2 text-xs text-[#404080]">Minggu {res.minggu}</span>}
              ...
              <div className="p-4 space-y-4">
                {res.monitoring?.enabled && (
                  <div>
                    ...
                    <table className="w-full border-collapse text-xs">
                      <thead><tr className="bg-[#EAEAF2] text-[#404080]"><th colSpan={2} className={`${cellPad} text-center`}>Jumlah Halaman</th><th colSpan={2} className={`${cellPad} text-center`}>Waktu Belajar</th><th className={cellPad}>Media</th><th className={cellPad}>Ketercapaian</th><th className={cellPad}>Penyebab</th><th className={cellPad}>Solusi</th></tr>
                        <tr className="bg-[#EAEAF2] text-[#404080]"><th className={cellPad}>Target</th><th className={cellPad}>Realisasi</th><th className={cellPad}>Target</th><th className={cellPad}>Realisasi</th><th className={cellPad}></th><th className={cellPad}></th><th className={cellPad}></th><th className={cellPad}></th></tr>
                      </thead>
                      <tbody><tr>
