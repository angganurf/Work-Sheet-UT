    ...
    <DashboardLayout nav={nav} title="Dashboard Admin" subtitle="Ringkasan seluruh aktivitas mahasiswa">
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-5 sm:p-6 mb-8">
        <div className="flex items-center gap-2 mb-1">
          <BarChart3 className="w-5 h-5 text-[#404080]" />
            ...
            <AreaChart data={chartData} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
              <defs>
                <linearGradient id="ut" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%" stopColor="#404080" stopOpacity={0.35} />
                  <stop offset="95%" stopColor="#404080" stopOpacity={0} />
                </linearGradient>
              </defs>
              ...
              <Tooltip
                ...
                labelStyle={{ color: "#404080", fontWeight: 700 }}
                formatter={(v) => [`${v} lembar kerja`, ""]}
              />
              <Area type="monotone" dataKey="value" stroke="#404080" strokeWidth={2.5} fill="url(#ut)" />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
        ...
        <div className="relative w-full sm:max-w-xs">
          ...
          <Input
            ...
            className="pl-10 bg-white focus:border-[#404080] focus:ring-1 focus:ring-[#404080]"
          />
        </div>
      </div>
      ...
      {rows === null ? (
        <div className="flex items-center justify-center py-20">
          <Loader2 className="w-7 h-7 animate-spin text-[#404080]" />
        </div>
      ) : rows.length === 0 ? (
              ...
              <tbody>
                {rows.map((w) => (
                  <tr key={w.id} className="border-t border-slate-100 hover:bg-slate-50/70 transition-colors" data-testid={`admin-row-${w.id}`}>
                    <td className="p-4 text-sm font-semibold text-[#404080]">{w.nim}</td>
                    ...
                    <td className="p-4 text-right">
                      <Button size="sm" variant="outline" onClick={() => navigate(`/admin/worksheet/${w.id}/view`)} data-testid={`admin-view-${w.id}`} className="border-slate-200 text-[#404080] hover:bg-[#EAEAF2]">
